...
(function() {
    ...
    function executeScript(scriptContent, scriptId) {
        ...
        var pv = document.getElementById("agentic-live-output");
    ...
    function sendResultToDeepSeek(output, exitCode, scriptId) {
        ...
        var truncated = output.length > 16000 ? output.slice(0, 16000) + '\n\n[... truncated ...]' : output;
    ...
    function updateResultsTab(output, exitCode) {
        ...
        var entry = '<div style="margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid #334155;"><div style="color:' + sc + ';font-weight:bold;">Exit: ' + exitCode + ' | ' + new Date().toLocaleTimeString() + '</div><pre style="background:#1e293b;padding:6px;border-radius:4px;max-height:200px;overflow-y:auto;margin:4px 0;font:11px monospace;color:#e2e8f0;">' + escapeHtml(output.slice(-6000)) + '</pre></div>';
rd.innerHTML = entry + rd.innerHTML;
    }
    ...
    function makeResizable(panel, handle, dir) {
        ...
        document.addEventListener("mousemove", function(e) {
            if (!r) return;
                    var w = dir === "tr" ? sw + (e.clientX - sx) : sw - (e.clientX - sx);
                    var h = sh - (e.clientY - sy);
                    w = Math.max(300, Math.min(1200, w));
                    h = Math.max(200, Math.min(window.innerHeight - 20, h));
                    if (dir === "tr") {
                        var oldRight = parseInt(panel.style.right) || 20;
                        panel.style.right = oldRight + "px";
                        panel.style.bottom = (parseInt(panel.style.bottom) || 20) + "px";
                    }
    ...
    function createPanel() {
        ...
        panel.innerHTML = '<div id="agentic-header"><span id="agentic-title">Agentic Bridge v2.8</span>' +
            '<div id="agentic-controls"><span id="agentic-status" title="Disconnected">🔴</span>' +
            ...
            '<button class="agentic-tab" data-tab="results">Results</button><button class="agentic-tab" data-tab="notepad">Notepad</button></div>' +
            '<div id="agentic-log" class="agentic-tab-content active"><div style="display:flex;height:100%;gap:0;"><div id="agentic-live-output" style="width:45%;overflow-y:auto;background:#020617;border-right:1px solid #334155;padding:4px 6px;font-size:10px;color:#a5b4fc;white-space:pre-wrap;word-break:break-all;flex-shrink:0;" title="Live output"></div><div id="agentic-log-inner" style="flex:1;overflow-y:auto;padding-left:4px;"></div></div></div>' +
            ...
            '<div id="agentic-resize-tl" style="position:absolute;top:0;left:0;width:14px;height:14px;cursor:nw-resize;background:linear-gradient(315deg,transparent 50%,#334155 50%);border-radius:12px 0 0 0;z-index:10;" title="Drag up-left"></div><div id="agentic-resize-tr" style="position:absolute;top:0;right:0;width:14px;height:14px;cursor:ne-resize;background:linear-gradient(45deg,transparent 50%,#334155 50%);border-radius:0 12px 0 0;z-index:10;" title="Drag up-right"></div>' +
        ...
        makeResizable(panel, document.getElementById("agentic-resize-tl"), "tl");
        makeResizable(panel, document.getElementById("agentic-resize-tr"), "tr");
        ...
        document.getElementById('agentic-clear').onclick = function() { document.getElementById("agentic-log-inner").innerHTML = ''; };
    ...
    function logPanel(text, type) {
        var log = document.getElementById("agentic-log-inner");
        if (!log) return;
    ...
    function updatePanelStatus(state) {
        ...
        if (!status) return;
        var icons = { connected: '🟢', disconnected: '🔴', error: '🟠' };
